      ...
      <div className="space-y-6">
        ...
        <div className="space-y-1.5 p-4 bg-primary/5 rounded-xl border border-primary/10">
          ...
          <div className="flex justify-between mt-1.5">
            ...
            <p className="text-[10px] text-muted-foreground font-bold">{completedTasks} / {tasks.length} Tasks</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-6 border-t border-b border-muted py-5 text-sm">
          ...
          <div className="flex items-start gap-2.5">
            ...
            <div>
              <p className="text-xs font-bold text-muted-foreground uppercase">{t('field.field')}</p>
              <p className="font-semibold text-foreground mt-0.5">
                {activity.fieldName} ({activity.farmName})
              </p>
              {activity.cropName && (
                <p className="text-xs text-muted-foreground font-medium mt-0.5">
                  {t('crop.crop')}: {activity.cropName}
                </p>
              )}
            </div>
          </div>
        ...
        <div className="space-y-3">
          <h5 className="text-xs font-bold text-foreground uppercase tracking-wider flex items-center gap-1.5">
            ...
            {isTa ? 'தொடர்புடைய பணிகள்' : 'Related Tasks'}
          </h5>
          {tasks.length === 0 ? (
            <p className="text-xs text-muted-foreground italic">No tasks created for this activity yet.</p>
          ) : (
            <div className="grid grid-cols-1 gap-2">
              {tasks.map(t => (
                <div key={t.id} className="p-3 bg-muted/10 border border-muted/50 rounded-lg flex items-center justify-between hover:bg-muted/30 transition-colors">
                  <div>
                    <p className="text-sm font-semibold text-foreground">{t.title}</p>
                    <p className="text-[10px] text-muted-foreground font-medium mt-0.5">Assigned to: {t.assignedToName || 'Unassigned'}</p>
                  </div>
                  <div className="flex flex-col items-end gap-1">
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${t.status === 'COMPLETED' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-muted text-muted-foreground'}`}>
                      {t.status}
                    </span>
