...
export default function DashboardScreen() {
  ...
  const farmId = user?.farmId || farms?.[0]?.id;
      ...
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="max-w-md bg-red-50 text-red-800 p-4 rounded-lg border border-red-200 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm mt-1">Failed to load dashboard data.</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="space-y-6">
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-emerald-700 via-emerald-600 to-teal-700 p-6 md:p-8 text-white">
          ...
          <p className="text-emerald-50/70 text-sm max-w-lg mb-6">
            {isTa ? 'உங்கள் பண்ணை இன்னும் அமைக்கப்படவில்லை' : 'Your farm is not set up yet.'}
          </p>
          <Button 
          ...
          >
            ...
            {isTa ? 'பண்ணையை அமைக்கவும்' : 'Set Up Farm'}
          </Button>
        </div>
      </div>
    );
      ...
      {isContextLoading || !farmContext ? (
        ...
        <>
          ...
          <div className="mb-2 mt-8 flex items-center justify-between">
            <h2 className="text-lg font-bold text-foreground">{isTa ? 'பண்ணை நுண்ணறிவு' : 'Farm Intelligence'}</h2>
          </div>
          
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <StatCard
              title={isTa ? 'செயலில் உள்ள பண்ணை' : 'Active Farm'}
            ...
            />
            <StatCard
              title={isTa ? 'வயல்கள்' : 'Field Count'}
            ...
            />
            <StatCard
              title={isTa ? 'கவனிக்க வேண்டியவை' : 'Attention Items'}
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="sf-card p-5">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <ActionBtn icon={HeartPulse} label={isTa ? 'ஆரோக்கியம்' : 'Farm Health'} onClick={() => navigate(`/farms/${farmId}/health`)} />
                  <ActionBtn icon={Leaf} label={isTa ? 'பயிர் பதிவு' : 'Record Crop'} onClick={() => navigate(`/farms/${farmId}`)} />
              ...
              <div className="sf-card p-5">
                ...
                <div className="space-y-4">
                  {farmContext.recentActivities.length > 0 ? (
                    ...
                    <p className="text-sm text-muted-foreground">{isTa ? 'செயல்பாடுகள் இல்லை' : 'No recent activities.'}</p>
                  )}
                </div>
              </div>
            </div>
              ...
              <div className="sf-card p-5">
                ...
                <div className="space-y-3">
                  {farmContext.attentionItems.length > 0 ? (
                    ...
                    <p className="text-sm text-muted-foreground">{isTa ? 'புதிய அறிவிப்புகள் இல்லை' : 'No new alerts.'}</p>
                  )}
                </div>
              </div>
            </div>
