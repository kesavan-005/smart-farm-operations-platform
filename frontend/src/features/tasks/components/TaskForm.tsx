    ...
    <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-6 bg-card border border-border p-6 rounded-xl shadow-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <div className="space-y-1.5">
          <label className="text-xs font-bold text-muted-foreground uppercase">{t('farm.farm')} *</label>
          <select
            {...register('farmId')}
            disabled={isWorker}
        ...
        {/* Field Select */}
        <div className="space-y-1.5">
          <label className="text-xs font-bold text-muted-foreground uppercase">{t('field.field')} *</label>
          <select
            {...register('fieldId')}
        ...
        {/* Activity Select */}
        <div className="space-y-1.5">
          <label className="text-xs font-bold text-muted-foreground uppercase">{t('activity.activity')} *</label>
          <select
            {...register('activityId')}
            disabled={isWorker || !watchedFarmId}
            className={`w-full h-9.5 rounded-lg border border-input bg-background px-3 py-1 text-sm shadow-sm transition-all focus-visible:outline-none ${
        ...
        {/* GPS Location */}
        <div className="space-y-1.5 md:col-span-2">
          <label className="text-xs font-bold text-muted-foreground uppercase">GPS Location</label>
          <Input
            type="text"
            placeholder="e.g. 11.1234, 77.5678"
            disabled={isWorker}
