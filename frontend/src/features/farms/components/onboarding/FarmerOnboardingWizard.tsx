    ...
    try {
      ...
      if (!serverFarmId) {
        ...
        const farmRes = await apiClient.post<ApiResponse<Farm>>('/farms', {
          name: farm.name,
          district: farm.district || primaryField?.district,
          taluk: farm.taluk || primaryField?.taluk,
          village: farm.village || primaryField?.village,
          state: farm.state || primaryField?.state || 'Tamil Nadu',
          address: farm.address || primaryField?.address,
          pincode: farm.pincode || primaryField?.pincode,
          latitude: farm.latitude ?? primaryField?.latitude ?? undefined,
          longitude: farm.longitude ?? primaryField?.longitude ?? undefined,
          boundary: farm.boundary || primaryField?.boundary || undefined,
          totalArea: farm.area,
          area: farm.area,
          areaUnit: primaryField?.areaUnit || 'Acres',
          soilType: farm.soilType,
          waterSource: farm.irrigationSource,
      ...
      console.error('Onboarding submission error:', err.response?.data || err);
      const details = err.response?.data?.details;
      let detailMsg = '';
      if (details && typeof details === 'object') {
        detailMsg = Object.entries(details).map(([k, v]) => `${k}: ${v}`).join(', ');
      }
      setError(detailMsg ? `Validation failed: ${detailMsg}` : (err.response?.data?.message || err.message || 'An error occurred during submission.'));
