  ...
  return useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        // Fallback for unsynced client-side farm IDs (e.g. 404 Not Found) or network error
        const farm = await db.table('farms').get(farmId);
        const fields = await db.table('fields').filter((f: any) => f.farmId === farmId).toArray();

        return {
          farmProfile: {
            name: farm?.name || 'Farm',
            location: farm?.district ? `${farm.district}, ${farm.state}` : (farm?.address || 'Local'),
            totalArea: farm?.totalArea || 0,
            areaUnit: farm?.areaUnit || 'Acres',
            soilType: farm?.soilType || 'N/A',
            irrigationSource: farm?.irrigationType || 'N/A',
          },
          summary: {
            totalFarmArea: farm?.totalArea || 0,
            areaUnit: farm?.areaUnit || 'Acres',
          ...
          cropStates: [],
          recentActivities: [],
          financeSummary: { currentMonthExpenses: 0, currency: 'INR' },
          inventorySummary: { totalUniqueItems: 0, lowStockCount: 0 },
          attentionItems: [],
        } as FarmContextResponse;
      }
    },
    enabled: !!farmId,
    staleTime: 5 * 60 * 1000,
