...
export default function FinanceDashboard({ dashboardData, transactions = [], isLoading, onNavigateToTab }: DashboardProps) {
  ...
  // Live computation from transactions array
  const now = new Date();
  ...
  (transactions || []).forEach((txn: any) => {
    ...
    const isToday = isValid && d >= todayStart && d <= todayEnd;
    const isThisMonth = isValid && d.getMonth() === currentMonth && d.getFullYear() === currentYear;

    if (txn.transactionType === 'REVENUE') {
      calcTotRev += amt;
      if (isToday) calcRevToday += amt;
      if (isThisMonth) calcRevMonth += amt;
      categoryRevMap[txn.category] = (categoryRevMap[txn.category] || 0) + amt;
      ...
      calcTotExp += amt;
      if (isToday) calcExpToday += amt;
      if (isThisMonth) calcExpMonth += amt;
      categoryExpMap[txn.category] = (categoryExpMap[txn.category] || 0) + amt;
  ...
  const revToday = (d.revenueToday !== undefined && d.revenueToday !== 0) ? d.revenueToday : calcRevToday;
  const expToday = (d.expensesToday !== undefined && d.expensesToday !== 0) ? d.expensesToday : calcExpToday;
  const revMonth = (d.revenueMonth !== undefined && d.revenueMonth !== 0) ? d.revenueMonth : calcRevMonth;
  const expMonth = (d.expensesMonth !== undefined && d.expensesMonth !== 0) ? d.expensesMonth : calcExpMonth;
  const netProfit = (d.netProfit !== undefined && d.netProfit !== 0) ? d.netProfit : (calcTotRev - calcTotExp);
  const cashAvailable = (d.cashAvailable !== undefined && d.cashAvailable !== 0) ? d.cashAvailable : calcCash;
  const inventoryValue = d.inventoryValue ?? 0;
  const outstandingPayments = (d.outstandingPayments !== undefined && d.outstandingPayments !== 0) ? d.outstandingPayments : calcOutstanding;
  const budgetUtilization = d.budgetUtilization ?? 0;
  const roi = d.roi ?? (calcTotExp > 0 ? Math.round(((calcTotRev - calcTotExp) / calcTotExp) * 100) : 0);

  const topExpensesList = (d.topExpenses && d.topExpenses.length > 0)
    ? d.topExpenses
    : Object.entries(categoryExpMap).map(([category, amount]) => ({ category, amount })).sort((a, b) => b.amount - a.amount).slice(0, 5);

  const topRevenueList = (d.topRevenue && d.topRevenue.length > 0)
    ? d.topRevenue
    : Object.entries(categoryRevMap).map(([category, amount]) => ({ category, amount })).sort((a, b) => b.amount - a.amount).slice(0, 5);

  const activitiesList = (d.activities && d.activities.length > 0)
    ? d.activities
    : (transactions || []).slice(0, 10).map((t: any) => ({
        ...
        category: t.category,
        amount: t.amount,
        refNumber: t.referenceNumber || t.id.substring(0, 8),
        farm: t.farmName || 'Farm',
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-card p-5 rounded-xl border border-border shadow-sm">
            ...
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={d.expenseBreakdown} innerRadius={45} outerRadius={60} paddingAngle={3} dataKey="value">
                  {d.expenseBreakdown?.map((_: any, idx: number) => (
          ...
          <div className="flex flex-wrap gap-2 text-[9px] font-semibold max-h-16 overflow-y-auto mt-2">
            {d.expenseBreakdown?.map((entry: any, idx: number) => (
        ...
        <div className="bg-card p-5 rounded-xl border border-border shadow-sm">
            ...
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={d.revenueSources} innerRadius={45} outerRadius={60} paddingAngle={3} dataKey="value">
                  {d.revenueSources?.map((_: any, idx: number) => (
          ...
          <div className="flex flex-wrap gap-2 text-[9px] font-semibold max-h-16 overflow-y-auto mt-2">
            {d.revenueSources?.map((entry: any, idx: number) => (
              <div key={idx} className="flex items-center gap-1">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-card p-5 rounded-xl border border-border shadow-sm flex flex-col justify-between">
          <div>
            ...
            <div className="space-y-3">
              {topExpensesList && topExpensesList.map((exp: any, idx: number) => (
              ...
              {(!topExpensesList || topExpensesList.length === 0) && (
                <p className="text-xs text-muted-foreground py-2">No expense entries recorded.</p>
              )}
            </div>
          </div>
          ...
          <div className="mt-4 pt-4 border-t border-border/30">
            ...
            <div className="space-y-3">
              {topRevenueList && topRevenueList.map((rev: any, idx: number) => (
                <div key={idx} className="flex items-center justify-between text-xs border-b border-border/50 pb-2">
              ...
              {(!topRevenueList || topRevenueList.length === 0) && (
                <p className="text-xs text-muted-foreground py-2">No revenue entries recorded.</p>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="bg-card p-5 rounded-xl border border-border shadow-sm lg:col-span-2">
          ...
          <div className="space-y-4 max-h-[350px] overflow-y-auto pr-1">
            {activitiesList && activitiesList.map((act: any) => {
              const isRev = act.transactionType === 'REVENUE';
              const dateVal = act.timestamp || act.transactionDate || act.createdAt;
              return (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-start">
                      ...
                      <span className={`font-black ${isRev ? 'text-emerald-600' : 'text-foreground'}`}>
                        {isRev ? '+' : '-'} ₹{Number(act.amount).toLocaleString()}
                      </span>
                    </div>
                    <div className="flex justify-between text-[10px] text-muted-foreground mt-0.5">
                      <span>Ref: #{act.refNumber || act.id?.substring(0, 8)} • {act.farm || 'Farm'}</span>
                      <span>{dateVal ? new Date(dateVal).toLocaleDateString() : 'Today'}</span>
                    </div>
                  </div>
                </div>
              );
            ...
            {(!activitiesList || activitiesList.length === 0) && (
              <p className="text-xs text-muted-foreground py-4 text-center">No recent financial movements.</p>
            )}
          </div>
        </div>
