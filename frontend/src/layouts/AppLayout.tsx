...
export function AppLayout({ children }: AppLayoutProps) {
  ...
  const handleLogout = async () => {
    try {
      const { logoutApi } = await import('@/features/auth/api/authApi');
      await logoutApi();
    } catch (e) {
  ...
  const handleFarmChange = (newFarmId: string) => {
    ...
    } else if (location.pathname.match(/\/farms\/[^/]+$/)) {
      ...
      >
        ...
        <div className="border-t border-border p-3 space-y-2 shrink-0">
          ...
          >
            ...
            {!isSidebarCollapsed && <span>{t('auth.logout', 'Logout')}</span>}
          </button>
        ...
        <header className="sticky top-0 z-[var(--z-sticky)] h-[var(--header-height)] bg-card/80 backdrop-blur-md border-b border-border flex items-center justify-between px-4 md:px-6 shrink-0">
          ...
          <div className="flex items-center gap-2 min-w-0">
            ...
            <span className="md:hidden text-sm font-semibold text-foreground capitalize truncate">
              {pathSegments[pathSegments.length - 1] || t('nav.dashboard', 'Dashboard')}
            </span>

            {/* Active Farm Selector Dropdown */}
            {farms && farms.length > 0 && (
              <div className="flex items-center gap-1.5 ml-2 pl-2 border-l border-border">
                <Tractor className="w-4 h-4 text-emerald-600 dark:text-emerald-400 shrink-0" />
                <select
                  value={currentActiveId || ''}
                  onChange={(e) => handleFarmChange(e.target.value)}
                  className="bg-card text-foreground border border-border rounded-md px-2 py-1 text-xs font-semibold focus:outline-none focus:ring-1 focus:ring-primary cursor-pointer max-w-[180px] truncate"
                  aria-label={t('farm.selectFarm', 'Select Farm')}
                >
                  {farms.map((f) => (
                    <option key={f.id} value={f.id} className="bg-card text-foreground">
                      {language === 'ta' && f.nameTa ? f.nameTa : f.name}
                    </option>
                  ))}
                </select>
              </div>
          ...
          <div className="flex items-center gap-1">
            ...
            {isOnline && pendingCount > 0 && (
              <span className="hidden sm:flex text-xs font-medium px-2.5 py-1 rounded-full bg-blue-500/10 text-blue-600 dark:text-blue-400 mr-1">
                {t('syncPending', { count: pendingCount, defaultValue: `Syncing ${pendingCount}` })}
              </span>
            )}
