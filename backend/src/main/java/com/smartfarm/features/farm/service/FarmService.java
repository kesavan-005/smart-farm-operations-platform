...
public class FarmService {
    ...
    public FarmResponse createFarm(FarmRequest request, UUID ownerId) {
        ...
        if (request.getBoundary() != null) {
            geofenceService.validateBoundary(request.getBoundary());
        }
        ...
        // Auto-calculate centroid if lat/lng are missing and boundary is present
        if ((farm.getLatitude() == null || farm.getLongitude() == null) && farm.getBoundary() != null) {
            double[] centroid = GeometryUtils.getCentroid(farm.getBoundary());
            if (centroid != null) {
                farm.setLatitude(BigDecimal.valueOf(centroid[0]));
                farm.setLongitude(BigDecimal.valueOf(centroid[1]));
            }
        }
